-- Fijar search_path en las funciones puras de cálculo (Capa 1)
ALTER FUNCTION public.fin_rentabilidad_pct(numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_ticket_promedio(numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_costo_fijo_por_servicio(numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_costo_variable_por_servicio(numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_ganancia_por_servicio(numeric, numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_punto_equilibrio(numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_costo_laboral_pct(numeric, numeric, numeric) SET search_path TO '';
ALTER FUNCTION public.fin_variacion_pct(numeric, numeric) SET search_path TO '';

-- generar_resumenes_mensuales: usar la fórmula central en vez de la copia inline
CREATE OR REPLACE FUNCTION public.generar_resumenes_mensuales(target_mes date DEFAULT NULL::date)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    FOR i IN 0..2 LOOP
      ...
      -- Mismos insumos que v_estadisticas_mensuales (ingresos activos del mes en huso local)
      ...
      -- Fórmula única de rentabilidad (Capa 1). No duplicar la cuenta acá.
      rent[i+1] := public.fin_rentabilidad_pct(fact[i+1], egr[i+1]);
    END LOOP;
    ...
    SELECT jsonb_object_agg(b.metodo, jsonb_build_object(
             ...
             'var_pct', CASE WHEN COALESCE(a.anterior,0) > 0
                             THEN ROUND(public.fin_variacion_pct(COALESCE(a.actual,0), a.anterior), 2)
                             ELSE NULL END))
      INTO v_metodos
